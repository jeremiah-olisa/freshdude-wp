...
class Apply {
    ...
    function login()
    {
        ...
        if((int)get_option('__wpdm_recaptcha_loginform', 0) === 1 && $csk != ''){
            $ret = wpdm_remote_post('https://www.google.com/recaptcha/api/siteverify', array('secret' => $csk, 'response' => wpdm_query_var('__recap')));
            $ret = json_decode($ret);
            if(!$ret->success) {
    ...
    function register()
    {
        ...
        if (!$user_id) {
            ...
            if (!$user_id) {
                ...
                if ((int)get_option('__wpdm_signup_autologin', 0) === 1) {
                    $reg_success = apply_filters("wpdm_signup_success", __("Your account has been created successfully.", "download-manager"));
                    wp_signon($creds);
                ...
                if (wpdm_is_ajax()) {
                    wp_send_json(array('success' => true, 'redirect_to' => add_query_arg(['signedup' => 1], wpdm_query_var('reg_redirect', 'url')), 'msg' => Session::get('__wpdm_signup_success')));
                    die();
                }
